                              ...
                              const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if (creature == followCreature || (creature == this && followCreature)) {
		if (hasFollowPath) {
			...
			int32_t MONSTERS_BEHAVIOR = g_config.getNumber(ConfigManager::MONSTERS_BEHAVIOR);
		
		
			if (MONSTERS_BEHAVIOR <= 0) { // max cpu, 0 ms reaction
				isUpdatingPath = false;
				g_dispatcher.addTask(createTask(std::bind(&Game::updateCreatureWalk, &g_game, getID())));
			} else if (MONSTERS_BEHAVIOR == 1) { // try to reduce cpu, fast reaction
				if (getWalkDelay() <= 0) {
					isUpdatingPath = false;
					g_dispatcher.addTask(createTask(std::bind(&Game::updateCreatureWalk, &g_game, getID())));
				} else {
			...
			} else if (MONSTERS_BEHAVIOR == 2) { // try to reduce cpu, fast reaction v2
				if (getWalkDelay() <= 0) {
					g_dispatcher.addTask(createTask(std::bind(&Game::updateCreatureWalk, &g_game, getID())));
				}
				isUpdatingPath = true;
			} else if (MONSTERS_BEHAVIOR == 3) {  // normal tfs
				isUpdatingPath = true;
			}
...
bool Creature::tryToFixAutoWalk(Direction dir)
{
	...
	while (!listWalkDir.empty() && attempts < 3) {
		std::forward_list<Direction> newPath;

		for (int i = 0; i < 2 && !listWalkDir.empty(); ++i) {
		...
		if (getPathTo(pos, newPath, 0, 0, false, true)) {
			newPath.reverse();
			listWalkDir.insert_after(listWalkDir.before_begin(), newPath.begin(), newPath.end());
			return true;
		}
