...
{
	CreatureEvent* creatureEvent { event };
	if (creatureEvent->getEventType() == CREATURE_EVENT_NONE) {
...
//{//onManaChange(creature, attacker, manaChange, origin)				//  OLD  DEPRECATED
	
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - CreatureEvent::executeManaChange] Call stack overflow" << std::endl;//  OLD  DEPRECATED
		return;
	}

	ScriptEnvironment* env = scriptInterface->getScriptEnv();//  OLD  DEPRECATED
	env->setScriptId(scriptId, scriptInterface);

	lua_State* L = scriptInterface->getLuaState();//  OLD  DEPRECATED
	scriptInterface->pushFunction(scriptId);

	LuaScriptInterface::pushUserdata(L, creature);
	LuaScriptInterface::setCreatureMetatable(L, -1, creature);//  OLD  DEPRECATED
	if (attacker) {
		LuaScriptInterface::pushUserdata(L, attacker);//  OLD  DEPRECATED
		LuaScriptInterface::setCreatureMetatable(L, -1, attacker);//  OLD  DEPRECATED
	} else {
		lua_pushnil(L);
	}
