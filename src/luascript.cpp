...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(CONST_ANI_BLUESTAR)
	registerEnum(CONST_ANI_GOLDENSTAR)
	...
	registerEnum(ITEM_ATTRIBUTE_BREAKCHANCE)
	...
	*/
	...
	registerEnumIn("configKeys", ConfigManager::EXAUSTED1)
	registerEnumIn("configKeys", ConfigManager::EXAUSTED2)
	registerEnumIn("configKeys", ConfigManager::MLDIVISOR)
	registerEnumIn("configKeys", ConfigManager::SKILLSDIVISOR)
	registerEnumIn("configKeys", ConfigManager::EXPDIVISOR)
	...
	registerEnumIn("configKeys", ConfigManager::abriu_nao_server_atacado)
	...
	registerEnumIn("configKeys", ConfigManager::MONSTERS_BEHAVIOR)
	registerEnumIn("configKeys", ConfigManager::FAST_ATTACK_SPEED)
...
// Action
	...
	registerMethod("Player", "sendOutfitWindowOTC", LuaScriptInterface::luaPlayerSendOutfitWindowOTC);
	...
	registerMethod("Player", "sendCastList", LuaScriptInterface::luaSendCastList);
	...
	registerMethod("ItemType", "getBreakChance", LuaScriptInterface::luaItemTypeGetBreakChance);
...
int LuaScriptInterface::luaPlayerSendOutfitWindowOTC(lua_State* L)
{
	// player:sendOutfitWindow()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
...
int LuaScriptInterface::luaDoPlayerSetSpectators(lua_State* L)
{
	...
	if (player) {
		...
		if (broadcast) {
			...
			player->sendChannel(CHANNEL_CAST, "Tibia Cast");
		}
...
int LuaScriptInterface::luaSendCastList(lua_State* L)
{
	//player:sendCastList()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
...
int LuaScriptInterface::luaItemTypeGetBreakChance(lua_State* L)
{
	// itemType:getExtraDefense()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		lua_pushnumber(L, itemType->breakChance);
	}
	else {
		lua_pushnil(L);
	}
