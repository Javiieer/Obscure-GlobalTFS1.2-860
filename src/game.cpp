...
bool Game::removeCreature(Creature* creature, bool isLogout/* = true*/)
{
	...
	for (Creature* summon : creature->summons) {
		addMagicEffect(summon->getPosition(), CONST_ME_POFF);
						   ...
						   uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count)
{
	...
	if (Creature* movingCreature = thing->getCreature()) {
		...
		//tempo pushar quanto esta perto push lado
		if (Position::areInRange<1, 1, 0>(movingCreature->getPosition(), player->getPosition())) {
			SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::EXAUSTED1),
...
void Game::playerMoveCreature(Player* player, Creature* movingCreature, const Position& movingCreatureOrigPos, Tile* toTile)
{
	...
	if (!Position::areInRange<1, 1, 0>(movingCreatureOrigPos, player->getPosition())) {
		...
		//delay to push along puxar longe tempo exausted primeira
		std::forward_list<Direction> listDir;
		if (player->getPathTo(movingCreatureOrigPos, listDir, 0, 1, true, true)) {
			g_dispatcher.addTask(createTask(std::bind(&Game::playerAutoWalk,
											this, player->getID(), listDir)));
			SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::EXAUSTED2), std::bind(&Game::playerMoveCreatureByID, this,
	...
	if (player != movingCreature) {
		if (toTile->hasFlag(TILESTATE_BLOCKPATH)) {
			...
			return;
		} else if ((movingCreature->getZone() == ZONE_PROTECTION) || (movingCreature->getZone() == ZONE_PROTECTION && !toTile->hasFlag(TILESTATE_PROTECTIONZONE)) || (movingCreature->getZone() == ZONE_NOPVP && !toTile->hasFlag(TILESTATE_NOPVPZONE))) {
			player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return;
		} else {
...
bool Game::removeMoney(Cylinder* cylinder, uint64_t money, uint32_t flags /*= 0*/)
{
	...
	for (const auto& moneyEntry : moneyMap) {
		...
		if (moneyEntry.first < money) {
			...
			addMoney(cylinder, (static_cast<unsigned long long>(worth) * removeCount) - money, flags);
...
void Game::addMoney(Cylinder* cylinder, uint64_t money, uint32_t flags /*= 0*/)
{
	...
	uint32_t SpecialCoins = money / 1000000;
	money -= static_cast<unsigned long long>(SpecialCoins) * 1000000;
	
	while (SpecialCoins > 0) {
		const uint16_t count = std::min<uint32_t>(100, SpecialCoins);

		Item* remaindItem = Item::CreateItem(ITEM_BAR_COIN, count);
...
void Game::playerRequestOutfitOTC(uint32_t playerId)
{
	if (!g_config.getBoolean(ConfigManager::ALLOW_CHANGEOUTFIT)) {
		return;
	}
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		if (realHealthChange > 0 && !target->isInGhostMode()) {
			...
			std::string healthChangeString = "+" + strHealthChange.str();
			addAnimatedText(healthChangeString, targetPos, TEXTCOLOR_RED);
			...
			SpectatorHashSet spectators;
			map.getSpectators(spectators, targetPos, false, true, 7, 7, 5, 5);
			for (Creature* spectator : spectators) {
		...
		if (target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
			...
			if (manaDamage != 0) {
				if (damage.origin != ORIGIN_NONE) {
					...
					if (!events.empty()) {
						for (CreatureEvent* creatureEvent : events) {
							//creatureEvent->executeManaChange(target, attacker, healthChange, damage.origin); //OLD
							creatureEvent->executeManaChange(target, attacker, damage);
						}
						healthChange = damage.primary.value + damage.secondary.value; //ADDED
						if (healthChange == 0) {
							return true;
						}
				...
				map.getSpectators(spectators, targetPos, true, true, 7, 7, 5, 5);
	...
	if (manaChange > 0) {
		if (attacker) {
			...
			if (attackerPlayer && attackerPlayer->getSkull() == SKULL_BLACK &&
				target->getPlayer() && attackerPlayer->getSkullClient(target) == SKULL_NONE) {
				return false;
			}
		}
		...
		if (realManaChange > 0 && !targetPlayer->isInGhostMode()) {
			...
			std::string manaChangeString = "+" + strManaChange.str();
			addAnimatedText(manaChangeString, target->getPosition(), TEXTCOLOR_BLUE);
		}
	}
	else {
		const Position& targetPos = target->getPosition();
