...
for (char ch : input) {
	...
	for (int hashByte = 20; --hashByte >= 0;) {
		...
		index = static_cast<size_t>(hashByte) << 1;
...
ShootTypeNames shootTypeNames = {
	...
	{"mix",			CONST_ANI_MIX},
	{"bluestar",			CONST_ANI_BLUESTAR},
	{"goldenstar",			CONST_ANI_GOLDENSTAR},
};
...
itemAbilityTypes statToAbilityPercent(uint8_t statid)
{
	...
	case STAT_MAGICPOINTS:
		return ITEM_ABILITY_MAXMAGICPOINTSPERCENT;
	default:
		return ITEM_ABILITY_NONE;
	}
}
...
itemAttrTypes stringToItemAttribute(const std::string& str)
{
	...
	else if (str == "breakchance") {
		return ITEM_ATTRIBUTE_BREAKCHANCE;
	}
	else if (str == "armor") {
