...
bool Monster::canSee(const Position& pos) const
{
    ...
    if(isMonster && std::abs(myPos.z - pos.z) > 1) {
        return false;
	}
...
{
	if (creature->getPlayer() && !creature->getPlayer()->hasClient()) {
		return true;
	}
...
void Monster::setIdle(bool idle)
{
	...
	//if (this->getPlayer() && !this->getPlayer()->hasClient()) {
		//idle = true;
		//Game::removeCreatureCheck(this);
		//clearTargetList();
		//clearFriendList();
		...
		if (!isIdle) {
			...
			if (isSummon()) {
				...
				if (!followCreature || !hasFollowPath) {
					//if (attackedCreature->getPlayer() && !attackedCreature->getPlayer()->hasClient()) {
						searchTarget();
					//}
					//std::cout << "3" << std::endl;
				} else if (isFleeing()) {
