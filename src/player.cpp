...
Player::Player(ProtocolGame_ptr p) :
	Creature(), lastPing(OTSYS_TIME()), lastPong(lastPing), inbox(new Inbox(ITEM_INBOX)), client(new Viewer(p)) 
{
	inMarket = false;
...
//std::cout << "hasLostConnection: " << hasLostConnection << std::endl;
//std::cout << "lastPing: " << lastPing << std::endl;
//std::cout << "noPongTime: " << noPongTime << std::endl;
//std::cout << "lastPing: " << lastPing << std::endl;						


	
	if (hasLostConnection && noPongTime >= 60000 && attackedCreature) {
		setAttackedCreature(nullptr);
	}

if ((!isConnecting) && !getTile()->hasFlag(TILESTATE_NOLOGOUT) && hasLostConnection == true) {
		if ((noPongTime >= g_config.getNumber(ConfigManager::WHITE_SKULL_TIME)) && getTile()->hasFlag(TILESTATE_PROTECTIONZONE) && hasCondition(CONDITION_INFIGHT) == 0 && (noPongTime >= 60000 && pzLocked == false)) {
...
{
	// Call the base class's blockHit function and store the result in blockType
	BlockType_t blockType = Creature::blockHit(attacker, combatType, damage, checkDefense, checkArmor, field);

	// If there is an attacker, send a message to darken the attacker's square
	if (attacker) {
		sendCreatureSquare(attacker, SQ_COLOR_BLACK);
	}

	// If the blockType is not BLOCK_NONE, return it immediately
	if (blockType != BLOCK_NONE) {
		return blockType;
	}

	// If the incoming damage is non-positive, set it to zero and return BLOCK_ARMOR
	if (damage <= 0) {
		damage = 0;
		return BLOCK_ARMOR;
	}
	...
	if (damage != 0) {
		...
		for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; ++slot) {
			// Check if the item ability for the current slot is enabled
			if (!isItemAbilityEnabled(static_cast<slots_t>(slot))) {
				continue; // Skip to the next slot if the ability is not enabled
			}

			// Get the item in the current slot
			Item* item = inventory[slot];
			if (!item) {
				continue; // Skip to the next slot if the slot is empty
			}

			// Get the item type information
			const ItemType& it = Item::items[item->getID()];

		// Check if the item has no abilities and the item type has no abilities
			if (!item->getAbilities() && !it.abilities) {
				...
				continue; // Skip to the next slot if there are no abilities
			}

			// Calculate the damage absorption percentage from the item's abilities
			const int16_t& absorbPercent = item->getAbilityValue(combatToAbsorb(combatType));
			if (absorbPercent != 0) {
				damage -= std::round(damage * (absorbPercent / 100.));

				// Reduce the charges of the item if it has charges
			...
			// If the field parameter is true, calculate field damage absorption
			if (field) {
		...
		// If there is an attacker, calculate reflected damage based on item abilities
			if (attacker) {
				const int16_t& reflectPercent = it.abilities->reflectPercent[combatIndex];
				if (reflectPercent != 0) {
					// Define combat parameters and reflect the damage back to the attacker
					//CombatParams params;
		...
		// If the total damage is non-positive, set it to zero and update the blockType to BLOCK_ARMOR
		if (damage <= 0) {
			damage = 0;
...
void Player::death(Creature* lastHitCreature)
{
	...
	if (skillLoss) {
		...
		manaSpent -= lostMana/g_config.getNumber(ConfigManager::MLDIVISOR);
		...
		for (uint8_t i = SKILL_FIRST; i <= SKILL_LAST; ++i) { //for each skill
			...
			uint32_t lostSkillTries = static_cast<uint32_t>(sumSkillTries * deathLossPercent/g_config.getNumber(ConfigManager::SKILLSDIVISOR));
		...
		//Level loss editado para perder menos
		uint64_t expLoss = static_cast<uint64_t>(experience * deathLossPercent/g_config.getNumber(ConfigManager::EXPDIVISOR));
...
{
	int32_t value;
	this->getStorageValue(69650, value);
	
	if (group->id >= 2 || value == 1) {
		return true;
	}
...
{	
	...
	this->getStorageValue(23232, value);
	
	
	if (hasFlag(PlayerFlag_NotGainInFight) || attacked == this || g_game.getWorldType() == WORLD_TYPE_PVP_ENFORCED || value >= 1) {
		return;
	}
...
double Player::getLostPercent() const
{
	...
	int32_t blessingCount = std::bitset<6>(blessings).count();
...
{
	int32_t value2;
	this->getStorageValue(69651, value2);

	if (group->id >= 2 || value2 == 1) {
		return true;
	}
