...
for (Item* item : itemList) {
		...
		if (owner) {
			...
			if (owner->getParty()) {
				...
				//owner->sendPrivateMessage(owner, TALKTYPE_PRIVATE, ss.str());
				owner->sendChannelMessage("", ss.str(), TALKTYPE_CHANNEL_O, 11);
			}
		}
	} else {
		std::ostringstream ss;
...
uint32_t Monsters::getLootRandom(Player* owner)
{
	
	
	
	double loot_extra = 1;
	double rate_loot = g_config.getNumber(ConfigManager::RATE_LOOT);

	if (owner) {
		
		// acaso a guild dominar o castle
		int32_t storage_castle = owner->getStorageValue(166001, storage_castle);
		if (storage_castle >= 1) {
			loot_extra = loot_extra+0.2; // 20%
		}
	

		double result;
		int32_t value;
		owner->getStorageValue(166000, value);
		if (value > 0) {
			if (value >= 15) {
				loot_extra = loot_extra+0.15; // +15% max
			} else {
				result = (static_cast<double>(value)/100);
				loot_extra = loot_extra+static_cast<double>(result);
			}
		}
		
		double result2;
		int32_t value2;
		owner->getStorageValue(166002, value2);
		if (value2 > 0) {
			if (value2 >= 10) {
				loot_extra = loot_extra+0.1; // +10% max
			} else {
				result2 = (static_cast<double>(value2)/100);
				loot_extra = loot_extra+static_cast<double>(result2);
			}
		}
	...
	return uniform_random(0, MAX_LOOTCHANCE) / rate_loot*loot_extra; // rate_loot quanto maior mais drop chance
}


std::vector<Item*> MonsterType::createLootItem(const LootBlock& lootBlock, Player* owner)
{

	int32_t itemCount = 0;
	uint32_t randvalue = Monsters::getLootRandom(owner);
	...
	while (itemCount > 0) {
		...
		if (lootBlock.breakChance != -1) {
			tmpItem->setIntAttr(ITEM_ATTRIBUTE_BREAKCHANCE, lootBlock.breakChance);
		}
...
bool MonsterType::createLootContainer(Container* parent, const LootBlock& lootblock, Player* owner)
{
...
bool Monsters::loadLootItem(const pugi::xml_node& node, LootBlock& lootBlock)
{
	...
	if ((attr = node.attribute("breakchance"))) {
		lootBlock.breakChance = pugi::cast<int32_t>(attr.value());
	}
