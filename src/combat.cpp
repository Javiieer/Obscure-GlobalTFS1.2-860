	...
	if (formulaType == COMBAT_FORMULA_DAMAGE) {
		...
		if (creature->getCombatValues(min, max)) {
			...
			if (params.valueCallback) {
				...
				if (weapon) {
					int32_t weaponDamage = weapon->getWeaponDamage(player, target, tool, true);
					damage.primary.value = normal_random(static_cast<int32_t>(minb), static_cast<int32_t>(weaponDamage * maxa + maxb));
	...
	if (casterPlayer && (damage.primary.value < 0 || damage.secondary.value < 0) &&
		...
		if (casterPlayer->getHealth() < casterPlayer->getMaxHealth()) {
			if (chance != 0 && skill > 0 && normal_random(1, 100) <= chance) {
				...
				lifeLeech.primary.value = std::abs(lifeLeech.primary.value);

				g_game.combatChangeHealth(nullptr, casterPlayer, lifeLeech);
				casterPlayer->sendMagicEffect(casterPlayer->getPosition(), CONST_ME_MAGIC_RED);
			}
		}

		// Mana Leech
		chance = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHCHANCE);
		skill = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHAMOUNT);
		...
		if (casterPlayer->getMana() < casterPlayer->getMaxMana()) {
			if (chance != 0 && skill > 0 && normal_random(1, 100) <= chance) {
				...
				manaLeech.primary.value = std::abs(manaLeech.primary.value);

				g_game.combatChangeMana(nullptr, casterPlayer, manaLeech);
				casterPlayer->sendMagicEffect(casterPlayer->getPosition(), CONST_ME_MAGIC_BLUE);
			}
		}

		// Critical
		chance = casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITCHANCE);
		skill = casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITAMOUNT);
